...
export function useProjects() {
  ...
  const query = useQuery({
    queryKey: [...PROJECTS_QUERY_KEY, userId] as const,
    queryFn: () => projectsService.getProjects(userId ?? undefined),
    enabled: Boolean(isAuthenticated && accessToken && userId),
    retry: (failureCount, error) => {
      if (error instanceof ProjectsApiError && error.status === 401) return false;
